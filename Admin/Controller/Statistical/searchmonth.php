<?php 
	...
	if(isset($_POST['query']))
	{
		...
		$query = "SELECT TenSP, HinhAnh, SUM(SoLuongMua) FROM hoadonchitiet INNER JOIN sanpham ON hoadonchitiet.MaSP = sanpham.MaSP INNER JOIN hoadon ON hoadon.MaHD = hoadonchitiet.MaHD WHERE MONTH(NgayDat) = MONTH('$search') AND YEAR(NgayDat) = YEAR('$search')  GROUP BY TenSP HAVING SUM(SoLuongMua) = (SELECT MAX(SoLuong) FROM ((SELECT SUM(SoLuongMua) AS SoLuong FROM hoadonchitiet GROUP BY MaSP) AS Temp))";

		$queryGetListProduct = "SELECT TenSP, SUM(SoLuongMua), SUM(hoadonchitiet.DonGia) AS TongTien FROM hoadonchitiet INNER JOIN sanpham ON hoadonchitiet.MaSP = sanpham.MaSP INNER JOIN hoadon ON hoadon.MaHD = hoadonchitiet.MaHD WHERE MONTH(NgayDat) = MONTH('$search') AND YEAR(NgayDat) = YEAR('$search') GROUP BY TenSP";
	}
	...
	if(mysqli_num_rows($sanPhamBanChay) > 0)
	{
		...
		$output .=	'</div><h3>Sản phẩm bán chạy nhất</h3><br />';
